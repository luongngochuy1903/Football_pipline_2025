...
def team_load():
    ...
    for team, league in from_map:
        ...
        df_league = spark.read.json(f"s3a://trusted/{league}/year={today.year}/month={today.month}/day={today.day}")
        ...
        for col_name in df_league.columns:
            
        # df.printSchema()
        # df.write.mode("append").parquet(f"s3a://refined/team/year={today.year}/month={today.month}/day={today.day}")